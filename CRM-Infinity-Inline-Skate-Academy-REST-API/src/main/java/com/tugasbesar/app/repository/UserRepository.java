...
public class UserRepository {
    ...
    public List<User> findMuridUsersForCoachLevelAndGrade(String coachUuid) {
        String sql = "SELECT u.uuid, u.full_name, u.username, u.email, u.password_hash, "
                + "u.role_uuid, r.name AS role, COALESCE(u.grade_uuid, l.grade_uuid) AS grade_uuid, COALESCE(student_grade.name, level_grade.name) AS grade_name, u.level_uuid, l.name AS level_name, u.is_super_admin, u.is_active, "
                + "u.last_login_at, u.created_at, u.updated_at "
                + "FROM users coach "
                ...
                + "JOIN users u ON u.uuid <> coach.uuid "
                + "JOIN levels l ON l.uuid = u.level_uuid "
                + "LEFT JOIN grades student_grade ON student_grade.uuid = u.grade_uuid "
                + "LEFT JOIN grades level_grade ON level_grade.uuid = l.grade_uuid "
                + "JOIN roles r ON r.uuid = u.role_uuid "
                + "WHERE coach.uuid = ? "
                + "AND u.is_super_admin = 0 "
                + "AND ("
                + "l.grade_uuid = COALESCE(coach_level.grade_uuid, coach.grade_uuid) "
                + "OR level_grade.grade_value = COALESCE(coach_level_grade.grade_value, coach_grade.grade_value) "
                + "OR LOWER(level_grade.name) = LOWER(COALESCE(coach_level_grade.name, coach_grade.name))"
                + ") "
